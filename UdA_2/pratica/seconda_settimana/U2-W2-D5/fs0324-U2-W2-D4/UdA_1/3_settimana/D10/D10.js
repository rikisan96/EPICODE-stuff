...
function stampaRisultato(risultato) {
  console.log(
    `Risultato Esercizio: ${risultato}, Esercizio N° ${seqEsercizio++}`
  );
}
...
function whoIsBigger(num1, num2) {
  return console.log(
    num1 > num2
      ? `Il numero maggiore e' ${num1} Esercizio N° ${seqEsercizio++}`
      : `Il numero maggiore e' ${num2} Esercizio N° ${seqEsercizio++}`
  );
}
...
console.log(
  `Esercizio N°`,
  seqEsercizio++,
  onlyLetters("Ciao sono 1 riccardo")
);
...
function isThisAnEmail(stringa) {
  ...
  verificaChiocciola + verificaDominio > 1
    ? console.log("Indirizzo MAIL VALIDO ")
...
seqEsercizio++;
stampaRisultato(
  `Giorni trascorsi dalla data del ${giornoDaCalcolare} - ${howManyDays()}  giorni `
);
...
function isTodayMyBirthday() {
  ...
  const oggi = new Date();
  return (
    oggi.getMonth() === mioCompleanno.getMonth &&
    oggi.getDay() === mioCompleanno.getDay()
  );
}
seqEsercizio++;
...
seqEsercizio++;
console.log(
  `Esercizio N° ${seqEsercizio} Ci sono `,
  countMovies(),
  ` films nell'oggetto 'movies'.`
);
...
function onlyTheYears(movies) {
  ...
  const releaseDates = movies.Year
